...
return true;
    ...
    // When tree loaded, add additional nodes if the user has paged to them
    var loadedListener = function ()
    {
      ...
      {
        getAndAppendNodes(currentTotalRootNodes, getLimit(), loadedListener);
      }
      else
      {
        updateMoreLink();
    ...
    {
      // If page has refreshed, deselect all nodes in case state has restored
      // selection
      if (refresh)
      {
    ...
    $fwTreeView
      .jstree(options)
      .bind('loaded.jstree', loadedListener)
